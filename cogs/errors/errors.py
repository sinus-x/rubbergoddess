    ...
    async def on_command_error(self, ctx: commands.Context, error):  # noqa: C901
        ...
        if isinstance(error, rubbercog.RubbercogException):
            if type(error) is not rubbercog.RubbercogException:
                return
            return await self.output.error(ctx, self.text.get("RubbercogException"), error)

        # Exceptions with parameters
        if type(error) == commands.MissingRequiredArgument:
            return await self.output.warning(ctx, self.text.get("MissingRequiredArgument", param=error.param))
        if type(error) == commands.CommandOnCooldown:
            time = utils.seconds2str(error.retry_after)
            return await self.output.warning(ctx, self.text.get("CommandOnCooldown", time=time))
        if type(error) == commands.MaxConcurrencyReached:
            return await self.output.warning(
                ctx,
                self.text.get("MaxConcurrencyReached", num=error.number, per=error.per.name)
            )
        if type(error) == commands.MissingRole:
            # TODO Is !r OK, or should we use error.missing_role.name?
            role = f"`{error.missing_role!r}`"
            return await self.output.warning(ctx, self.text.get("MissingRole", role=role))
        if type(error) == commands.BotMissingRole:
            role = f"`{error.missing_role!r}`"
            return await self.output.error(ctx, self.text.get("BotMissingRole", role=role))
        if type(error) == commands.MissingAnyRole:
            roles = ", ".join(f"`{r!r}`" for r in error.missing_roles)
            return await self.output.warning(ctx, self.text.get("MissingAnyRole", roles=roles))
        if type(error) == commands.BotMissingAnyRole:
            roles = ", ".join(f"`{r!r}`" for r in error.missing_roles)
            return await self.output.error(ctx, self.text.get("BotMissingAnyRole", roles=roles))
        if type(error) == commands.MissingPermissions:
            perms = ", ".join(f"`{p}`" for p in error.missing_perms)
            return await self.output.warning(ctx, self.text.get("MissingPermissions", perms=perms))
        if type(error) == commands.BotMissingPermissions:
            perms = ", ".join(f"`{p}`" for p in error.missing_perms)
            return await self.output.error(ctx, self.text.get("BotMissingPermissions", perms=perms))
        if type(error) == commands.BadUnionArgument:
            return await self.output.warning(ctx, self.text.get("BadUnionArgument", param=error.param.name))

        # The rest of client exceptions
        if isinstance(error, commands.ClientException):
            return await self.output.warning(ctx, self.text.get(type(error).__name__))

        # All cog-related errors
        if isinstance(error, commands.ExtensionError):
            return await self.output.error(ctx, self.text.get(type(error).__name__, extension=f"{error.name!r}"))
        ...
        if type(error) in (discord.NoMoreItems, discord.HTTPException, discord.Forbidden, discord.NotFound):
            return await self.output.error(ctx, self.text.get(type(error).__name__))
